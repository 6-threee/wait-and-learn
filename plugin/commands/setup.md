---
description: Install the Agora flashcard status line (copies the runtime to a stable location and wires it into your Claude Code settings, keeping any existing status line).
---

Set up the Agora language-flashcard status line for this user.
...
The script copies the runtime to `~/.agora/runtime/`, backs up the
...
and grade the word currently on screen with `/agora:wl got` (knew it) or
`/agora:wl missed` (didn't).
